...
class ConfigRepository():
    ...
    def setFullPath(self, config):
        ...
        epFullPath = '{root}/{ep_path}/ep_catalogid_{catalog_id}.{file_format}'.format(
            root=root, 
            ep_path=self.envConfig('ep_path'), 
            catalog_id=config['info']['catalog_id'], 
            file_format=config['ep']['format']
        )
        
        feedFullPath = '{root}/{feed_path}/feed_catalogid_{catalog_id}.{file_format}'.format(
            root=root,
            feed_path=self.envConfig('feed_path'), 
            catalog_id=config['info']['catalog_id'], 
            file_format='tsv'
        )
